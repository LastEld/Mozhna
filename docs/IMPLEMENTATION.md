...
2. `My Workspace`, бюджет €0 и отсутствие карты подтверждены. Free web и PostgreSQL созданы; сборка Render прошла. Заполнить `DATABASE_URL` из Dashboard БД по [DEPLOYMENT](DEPLOYMENT.md), выполнить deployment, затем проверить HTTPS и background job. Текущий deploy завершился `update_failed`; приложение ещё не доступно для использования.
...
Commit `1596a5583bf46be125e4208a99e6e2883e160f39` прошёл [Runtime CI](https://github.com/LastEld/Mozhna/actions/runs/34344638115): 72 backend tests без пропусков, 10 frontend tests, PostgreSQL migration/restore, Docker smoke, native Free build и реальный supervisor/job/restart smoke. Browser suite: 8 успешных проверок и 1 ожидаемый сбой WebKit offline; это не закрывает iPhone offline gate. На Render та же версия успешно собрана, но HTTPS/runtime-проверки заблокированы отсутствующим подключением БД.

## Бесплатный тестовый runtime

По поручению владельца платная Render-топология заменена на Free web + Free PostgreSQL. API/worker/scheduler — отдельные процессы одного экземпляра; supervisor применяет migration, останавливает все роли при сбое, обрабатывает shutdown и запрещает платный inference. Idle web останавливает фон, jobs хранятся в PostgreSQL до её истечения через 30 дней. Это ограниченный тест, не always-on production. Отсутствие карты подтверждено владельцем; биллинг коннектором не проверяется. Оба Free-ресурса созданы, но запуск остановлен из-за отсутствующего `DATABASE_URL`; БД истекает 2026-10-09 в 12:44:12 UTC. [Детали](DEPLOYMENT.md).
